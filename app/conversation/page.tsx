        ...
        <div className="flex justify-center gap-3">
          {([
            ...
            <button key={item.lang} onClick={() => selectLanguage(item.lang)}
              className="flex items-center gap-2.5 py-3 px-6 rounded-[10px] bg-white dark:bg-[#111] hover:ring-2 hover:ring-[hsl(160,84%,42%)] transition-all duration-150 cursor-pointer group"
              style={{ boxShadow: "var(--shadow-card)" }}>
              <FlagImg lang={item.lang} size={22} />
              <span className="text-[14px] font-semibold text-[#171717] dark:text-[#f5f5f5] group-hover:text-[hsl(160,84%,42%)] transition-colors">{item.name}</span>
            </button>
          ))}
        </div>
      </div>
